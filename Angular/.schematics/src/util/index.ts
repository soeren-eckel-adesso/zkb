...
export function createLib(options: any): Rule {
  return (tree: Tree, context: SchematicContext) => {
    ...
    options.path = `libs/@${options.customer}/utils/${options.name}`;
...
function updateAngularJson(options: any, tree: Tree, context: SchematicContext) {
  ...
  const angularConfig = tree.read("/.schematics/src/util/angular.json");
  ...
  try {
    ...
    if (angularJsonFileObject && newAngularJsonFileObject) {
      angularJsonFileObject["projects"]["@" + options.customer + "/util-" + options.name] = newAngularJsonFileObject;
...
function updateTsConfig(options: any, tree: Tree, context: SchematicContext) {
  ...
  try {
    ...
    if (angularJsonFileObject) {
      angularJsonFileObject["compilerOptions"]["paths"]["@" + options.customer + "/util-" + options.name] = [
        "libs/@" + options.customer + "/utils/" + options.name + "/src/public-api",
      ];
