...
export function createLib(options: any): Rule {
  return (tree: Tree, context: SchematicContext) => {
    ...
    options.path = `libs/@${options.customer}/uis/${options.name}`;
...
function updateAngularJson(options: any, tree: Tree, context: SchematicContext) {
  ...
  const angularConfig = tree.read("/.schematics/src/ui/angular.json");
  ...
  try {
    ...
    if (angularJsonFileObject && newAngularJsonFileObject) {
      angularJsonFileObject["projects"]["@" + options.customer + "/ui-" + options.name] = newAngularJsonFileObject;
...
function updateTsConfig(options: any, tree: Tree, context: SchematicContext) {
  ...
  try {
    ...
    if (angularJsonFileObject) {
      angularJsonFileObject["compilerOptions"]["paths"]["@" + options.customer + "/ui-" + options.name] = [
        "libs/@" + options.customer + "/uis/" + options.name + "/src/public-api",
      ];
