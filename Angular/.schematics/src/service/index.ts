...
export function createLib(options: any): Rule {
  return (tree: Tree, context: SchematicContext) => {
    ...
    options.path = `libs/@${options.customer}/services/${options.name}`;
...
function updateAngularJson(options: any, tree: Tree, context: SchematicContext) {
  ...
  const angularConfig = tree.read("/.schematics/src/service/angular.json");
  ...
  try {
    ...
    if (angularJsonFileObject && newAngularJsonFileObject) {
      angularJsonFileObject["projects"]["@" + options.customer + "/service-" + options.name] = newAngularJsonFileObject;
...
function updateTsConfig(options: any, tree: Tree, context: SchematicContext) {
  ...
  try {
    ...
    if (angularJsonFileObject) {
      angularJsonFileObject["compilerOptions"]["paths"]["@" + options.customer + "/service-" + options.name] = [
        "libs/@" + options.customer + "/services/" + options.name + "/src/public-api",
      ];
